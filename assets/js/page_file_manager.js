...
//======================================================================================================================
    ...
    $('.app-modal-new-trigger').on('click', function() {
        initNew(this);
    });
...
//======================================================================================================================
    ...
    function initNew(trigger)
    {
        let $modal = $('#app-modal-new');
        let $radioButtons = $modal.find('input[name="type"]');
        let $showWithLink = $modal.find('.app-show-with-link');

        let $directoryBrowserPathInput = $modal.find('.app-directory-browser-container input[name="path"]');
        ...
        $radioButtons.change(function() {
            ...
            if ($this.is(':checked'))
            {
                if ($this.val() === 'link')
                {
                    ...
                    $directoryBrowserPathInput.trigger('change');
                    $directoryBrowserPathInput.prop('required', true);
                    $showWithLink.show(200);
                }
                else
                {
                    $showWithLink.hide(200);
                    $directoryBrowserPathInput.prop('required', false);
                }
            }
        }).trigger('change');
...
//======================================================================================================================
    ...
    function initCopyMoveSingle(trigger)
    {
        let config = readConfiguration(trigger);
        let items = [];
        items.push(config.id);
        initCopyMove(items);
    }

    function initCopyMoveBulk()
    {
        let items = [];
    ...
    function initCopyMove(items)
    {
        var $modal = $('#app-modal-copy-move');
        var $selectedFiles = $modal.closest('form').find('.app-modal-selected-files');

        let $directoryBrowserContainer = $modal.find('.app-directory-browser-container');
        let $directoryBrowserBrowseButton = $directoryBrowserContainer.find('.app-browse-directory');
        let $directoryBrowser = $directoryBrowserContainer.find('.app-directory-browser');
        ...
        // Reset modal to default state
            ...
            // Set start directory to /www/
            $directoryBrowserContainer.find('input[name="path"]').val('/www/');

            // Open directory browser.
            if ($directoryBrowser.is(':hidden'))
            {
                $directoryBrowserBrowseButton.trigger('click');
            }
