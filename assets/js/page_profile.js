...
//======================================================================================================================
    ...
    $('#app-add-authenticator').click(function() {
        $('#app-add-authenticator').addClass('is-loading');
        $('#app-webauthn-error').hide();
        $('#app-webauthn-error-message').text('');

        createWebAuthnRegistration();
    });

    // Read the settings from the items table row and assign them to the modal.
    $('.app-delete-webauthn-authenticator').on('click', function() {
        let $this = $(this);
        let $modal = $('#app-modal-delete-webauthn-authenticator');
        ...
        $form.find('.app-webauthn-authenticator-name').html(name);

        $modal.addClass('is-active');
    });

    // Read the settings from the items table row and assign them to the modal.
    $('.app-update-webauthn-authenticator').on('click', function() {
        let $this = $(this);
        let $modal = $('#app-modal-update-webauthn-authenticator');
        let $form = $modal.closest('form');
        let $tr = $this.closest('tr');
        let id = $tr.find('input[name="id"]').val();
        let name = $tr.find('input[name="name"]').val();
