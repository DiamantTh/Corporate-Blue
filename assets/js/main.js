...
//======================================================================================================================
    ...
    if (!hash['white_label.is_control_panel'])
    {
...
//======================================================================================================================
    ...
    // Click on the card icons should not toggle the collapse.
    $('.card-header.is-collapsible .app-header-buttons').on('click', function(e) {
        e.stopPropagation();
    });
...
//======================================================================================================================
    ...
    // Select / Deselect all checkboxes within a table at once.
...
//======================================================================================================================
    ...
    // Display password annotations.
    $('input[name="password"]').on('change keyup', function(event) {
        var value = $(this).val();
        var $parentContainer = $('#password-annotations');
        var annotations = new Object();

        // Complexity
        // Do not use keyword 'length' in complexity array.
        annotations['min-length'] = value.length < hash['password.complexity.length'];
        annotations['letter']     = hash['password.complexity.letter']    && !value.match(/[a-zA-Z]+/);
        annotations['lowercase']  = hash['password.complexity.lowercase'] && !value.match(/[a-z]+/);
        annotations['uppercase']  = hash['password.complexity.uppercase'] && !value.match(/[A-Z]+/);
        annotations['digit']      = hash['password.complexity.digit']     && !value.match(/[0-9]+/);
        // Underscore not matched by \W.
        annotations['special']    = hash['password.complexity.special']   && !value.match(/[_\W]+/);
        // Misc
        // Check for event type! Autocompletion may interfere (on login, and 'Generate password').
        annotations['caps-lock'] = event.originalEvent instanceof KeyboardEvent && event.originalEvent.getModifierState('CapsLock');

        // Show/Hide annotations texts.
        for (var key in annotations)
        {
            var selector = '.app-password-annotation-' + key;
            annotations[key] ? $(selector).show(200) : $(selector).hide();
        }
...
//======================================================================================================================
// Reset default for input fields
//======================================================================================================================

    // Reset to default value, when clicking the 'reset' button next to an input field.
    $('.app-reset-default').on('click', function() {
        let $this = $(this);
...
//======================================================================================================================
    ...
    $('button[data-submit-form]').on('click', function() {
        const selector = '#' + $(this).attr('data-submit-form');
        $(selector).trigger('submit');
    });

    // Submit a form using the bulk actions drop down.
    $('.app-update-action-submit-form').on('click', function() {
        const $this = $(this);
        ...
        const $form = $('#' + $this.attr('data-form-id'));
        ...
        const newActionUrl = new URL($form.attr('action'), window.location.origin);
        newActionUrl.searchParams.set('action', actionId);
        $form.attr('action', newActionUrl.toString());

        const modalId = $this.attr('data-open-modal-on-' + actionId);
        const functionName = $this.attr('data-call-function-on-' + actionId);
...
$("textarea.app-auto-resize").each(function () {
    ...
    {
        // Textarea is visible.
        // - It has been rendered, therefor scrollHeight will be bigger than zero.
        // - We use the height attribute to manage the auto-resize.
    ...
    else
    {
        ...
        // - It has not been rendered and therefor scrollHeight will be zero.
        // - We use rows, to make them appear as big as it needed, the first time they are rendered.
        // - Later on the input function, when the textarea receive regular input, we switch to managing auto-resize with the height attribute, like above.
        const minimumRows = this.rows;
        const rows = this.value.split('\n').length;
        const limitedRows = Math.min(Math.max(rows, minimumRows), maxLines);

        this.dataset.minimumRows = minimumRows;
        this.rows = limitedRows;
        this.style.overflowY = rows > maxLines ? 'auto' : 'hidden';
    }
}).on('input', function ()
{
    const maxHeight = parseFloat(this.dataset.maxHeight);
    const minimumRows = parseInt(this.dataset.minimumRows);

    if (minimumRows > 0)
    {
        this.rows = this.dataset.minimumRows;
    }

    // Set height to auto to get the real scrollHeight.
    this.style.height = "auto";
    const newHeight = Math.min(this.scrollHeight, maxHeight);
    const overflow = this.scrollHeight > maxHeight ? 'auto' : 'hidden';
...
//======================================================================================================================
    ...
    var tippySettingsTooltipMandatory = {
        ...
        // Eg if called inside of the index tables, the postion of the tooltip may not work right,
...
//======================================================================================================================
// Additional content | Reveal / Collapse content
//======================================================================================================================

    $('.app-additional-content-toggle').on('click', function() {
       const $this = $(this);
       const $showToggle = $this.find('.app-additional-content-toggle-show');
       const $hideToggle = $this.find('.app-additional-content-toggle-hide');
       const $content = $('#' + $this.data('target-id'));
