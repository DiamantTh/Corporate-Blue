...
# --- CSS bauen (FontAwesome + Tailwind, falls Toolchain vorhanden) -----------
#  fontawesome.css + tw.css werden in assets/css/ erwartet. Wenn package.json
#  vorhanden ist, frisch builden — andernfalls Existenz prüfen.
if [[ -f "package.json" ]] && command -v npm >/dev/null 2>&1; then
    echo "CSS: baue assets/css/fontawesome.css + assets/css/tw.css ..."
    if [[ ! -d "node_modules" ]]; then
        npm install --silent --no-audit --no-fund
    fi
    npm run build --silent
fi
if [[ ! -f "assets/css/fontawesome.css" ]]; then
    echo "FEHLER: assets/css/fontawesome.css fehlt — 'npm run build:fa' vor Release ausführen." >&2
    exit 1
fi
if [[ ! -f "assets/css/tw.css" ]]; then
    echo "FEHLER: assets/css/tw.css fehlt — 'npm run build:css' vor Release ausführen." >&2
    exit 1
fi
